...
The LLM runs entirely inside Internet Computer canisters. There is no API call
to an off-chain model anywhere: the weights live in the canister's stable
memory, and every token is generated by on-chain compute. Prompt caching also
happens in the canister, in stable memory, with each user's cache isolated.
