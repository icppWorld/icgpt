# ICGPT - the OG of on-chain LLMs

**ICGPT was released in September 2023, as the very first app to run LLM
inference on-chain, and it has been serving on-chain LLMs ever since.**

See the original announcement on the
[DFINITY forum](https://forum.dfinity.org/t/llama2-c-llm-running-in-a-canister/21991/16),
September 17, 2023.
...
memory, and every token is generated by on-chain compute.

Prompt caching also happens in the canister, in stable memory, and every user
gets their own isolated cache. Each turn re-uses what the LLM has already
processed instead of re-reading the whole conversation, which saves compute and
makes longer chats possible.
